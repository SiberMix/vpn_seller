...
- Файлы с данными будут размещены по адресу `/var/lib/vanish_vpn`.
...
Для запуска с помощью linux systemctl (скопируйте файл marzban.service в `/var/lib/vanish_vpn/marzban.service`):
...
1. По умолчанию все важные файлы Marzban сохраняются в папке `/var/lib/vanish_vpn` (в версиях Docker). Скопируйте весь каталог `/var/lib/vanish_vpn` в выбранное вами место резервного копирования, например на внешний жесткий диск или в облачное хранилище.
